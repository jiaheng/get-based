...
- **`js/`** — 50 ES modules loaded via `js/main.js`:
  ...
  - `lens.js` — Knowledge Base backing the Interpretive Lens. `queryLens` retrieves top-K, `injectLensChunks` folds into `[section:interpretiveLens]`. LRU cache (20/5min, profile-scoped), tight fetch options. Three backends selected via radio in Settings → AI → Knowledge Base: **`in-browser`** (OPFS + MiniLM via `lens-local*`, default), **`desktop-engine`** (bundled Python backend on `127.0.0.1:8322`, desktop app only), **`external-server`** (user URL + Bearer key). `migrateLensConfig()` maps legacy `remote`/`local-browser` values. Settings renders inline ingest UI + library picker for `in-browser`, mounts `renderKnowledgeBaseSection()` for `desktop-engine`, URL/key fields for `external-server`
