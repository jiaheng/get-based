...
2. *Per-row CRDT deltas* — the new path. Three planners (`_planArrayDelta`, `_planKeyedMapDelta`, `_planScalarDelta`) emit small per-item inserts/updates/tombstones into a generic `itemRow` table keyed by `arrayName + itemId`. Covers **44 importedData surfaces**: 13 arrays (`DELTA_ARRAYS` — sunSessions, lightDevices, deviceSessions, lightAudits, lightMeasurements, lightEnvironment.rooms, lightEnvironment.screens, entries, notes, supplements, healthGoals, changeHistory, chatSummaries), 10 keyed maps (`DELTA_MAPS` — markerNotes, customMarkers, manualValues, refOverrides, categoryLabels, categoryIcons, markerLabels, wearablePrimaryOverride, genetics.snps, lightDailyVerdicts), and 21 scalars (`DELTA_SCALARS` — 8 context cards + interpretiveLens + contextNotes + menstrualCycle + emfAssessment + genetics + biometrics + sunCorrelations + lifelightProfile + sunDefaults + channelMixAI + lightEnvironment.burdenAI + wearableSummary + wearableCardOrder).
