...
Hosted on Mintlify at `docs.getbased.health`. `vercel.json` 301-redirects the old `app.getbased.health/docs/*` paths to the Mintlify equivalents (`/docs/guide/X` → `/guides/X`, with a few explicit renames). The legacy VitePress source still lives in `docs/` — kept only because `tests/test-wearables.js` reads `docs/guide/wearables.md` directly; it is no longer built or deployed.
