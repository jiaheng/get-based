...
`robots.txt` is static; `sitemap.xml` is generated by `scripts/gen-sitemap.mjs` from `dist-docs/` (wired into `vercel.json`'s `buildCommand`, after `docs:build`) and is gitignored like other build output.
