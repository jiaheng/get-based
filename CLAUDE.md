...
- **`js/`** — ES modules loaded via `js/main.js` (count drifts; `ls js/*.js | wc -l`). Grouped by concern below; read the source for exported symbols.
  ...
  - **Chat + context**: `chat.js` (public barrel + entry point), `chat-window-bindings.js` (callback wiring and legacy `window.*` exports), `chat-marker-prompts.js` (per-marker and selected-correlation prompt builders), `chat-send.js` (direct chat send, streaming, stop-button, typewriter, image send integration), `chat-render.js` (transcript rendering), `chat-empty-state.js` (empty chat and chat-first onboarding message HTML), `chat-panel.js` (panel open/close/fullscreen, web-search toggle, input state), `chat-nudge.js` (FAB nudge badge and dismissal state), `chat-onboarding.js` (chat-first onboarding CTAs, profile/cycle/supplement handlers, and provider quiz helpers), `chat-discussion.js` (public discussion barrel), `chat-discussion-flow.js` (multi-persona discussion user-action handlers), `chat-discussion-callbacks.js` (discussion callback bridge), `chat-discussion-lifecycle.js` (discussion cleanup and completion helpers), `chat-discussion-turns.js` (discussion turn execution helpers), `chat-discussion-state.js` (discussion persona/thread state helpers), `chat-discussion-round-runner.js` (per-persona discussion round execution loop), `chat-discussion-round-prompts.js` (discussion round prompt helpers), `chat-discussion-round-request.js` (discussion round API request helpers), `chat-discussion-round-state.js` (thread-bound discussion round persistence), `chat-discussion-round-view.js` (live discussion round DOM helpers), `chat-discussion-picker.js` (discussion persona picker controls), `chat-discussion-ui.js` (discussion button and continuation prompt controls), `chat-personalities.js` (personality selection, custom persona editor, chat header status), `chat-history.js` (thread-aware chat history load/save/clear), `chat-threads.js` (thread index CRUD and rail rendering), `chat-thread-search.js` (thread rail message search and match highlighting), `chat-actions.js` (message action bar rendering + handlers), `chat-icons.js` (chat SVG icons + icon-button DOM helper), `chat-attestation.js` (Venice E2EE lock/attestation markup), `chat-continuation.js` (token-limit detection + auto-continue wrapper), `chat-prompt-context.js` (persona prompt, web hint, tagged-history, and lens-source helpers), `chat-summaries.js` (conversation summaries + saved-summary modal actions), `lab-context.js` (extracted: context assembly + memoization), `markdown.js` (extracted: XSS-safe rendering)
