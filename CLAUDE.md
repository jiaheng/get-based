...
- **`js/`** — 56 ES modules loaded via `js/main.js`. Grouped by concern below; read the source for exported symbols.
  ...
  - **Wearables**: `wearable-adapters.js` (canonical metric registry + per-vendor endpoint map), `wearables-oura-auth.js` (OAuth2 server-side flow, token exchange via `/api/proxy`, refresh middleware), `wearables-oura.js` (Oura data layer, 8 endpoints: sleep / daily_sleep / daily_readiness / daily_activity / daily_stress / daily_resilience / daily_cardiovascular_age / daily_spo2), `wearables-store.js` (per-profile IndexedDB `labcharts-wearables-{profileId}`, never syncs), `wearables-summary.js` (L2 derivation + write gate — 5% d7 shift, trend flips, week rollovers, 14d force-refresh), `wearables-connect.js` (connect/disconnect/backfill/sync orchestration), `wearables.js` (dashboard strip render + settings panel)
