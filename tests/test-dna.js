...
const annotatedReportCsv =
  ...
  'rs429358,APOE,Alzheimer/Cardiovascular,-,-,-,e3/e3,-,APOE part 1,variant-VCF\n' +
  'rs7412,APOE,Alzheimer/Cardiovascular,-,-,-,e3/e3,-,APOE part 2,variant-VCF\n' +
  ...
  'rs1800562,HFE,Iron Metabolism,het,Heterozygous,A,Carrier,-,C282Y carrier,variant-VCF\n';
assert('Detects annotated SNP report CSV', dna.detectDNAFile(annotatedReportCsv) === 'annotated-snp-report');
...
const annotatedFile = new File([annotatedReportCsv], 'results.csv', { type: 'text/csv' });
const annotatedResult = await dna.parseDNAFile(annotatedFile);
assert('Annotated SNP report: source detected', annotatedResult.source === 'Annotated SNP report');
assert('Annotated SNP report: APOE e3/e3 part 1 inferred', annotatedResult.matches.rs429358?.genotype === 'TT');
assert('Annotated SNP report: APOE e3/e3 part 2 inferred', annotatedResult.matches.rs7412?.genotype === 'CC');
assert('Annotated SNP report: wildtype inferred as catalog reference', annotatedResult.matches.rs1801133?.genotype === 'GG');
...
assert('DNA import save failure resets running flag but keeps preview retryable',
  ...
  !dnaSaveFailureBlock.includes('_pendingDNAImport = null'));
